...
fn allocatingMemory() !void
{
    ...
    const fb_allocator:std.mem.Allocator = fba.allocator();

    const memory = try fb_allocator.alloc(u8, 100); //then we create the memory
    memory[0] = 'a';
    memory[1] = 'b';
    memory[2] = 'c';
    memory[3] = 'd';
    ...
    defer fb_allocator.free(memory); //then we free the data when out of scope
